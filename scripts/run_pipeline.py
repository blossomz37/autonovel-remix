...
def run_tool(cmd: str | list, timeout: int = 600, check: bool = False) -> subprocess.CompletedProcess:
    """
    Run a tool as a subprocess, capturing output.
    Automatically splits string commands using shlex.
...
def uv_run(script: str, timeout: int = 600) -> subprocess.CompletedProcess:
    """Shorthand for 'uv run python scripts/<script>' from project root."""
    return run_tool(f"uv run python scripts/{script}", timeout=timeout)
        ...
        for attempt in range(1, MAX_CHAPTER_ATTEMPTS + 1):
            ...
            if score >= CHAPTER_THRESHOLD:
                ...
                if ch_file.exists():
                    try:
                        subprocess.run(
                            ["git", "checkout", "--", f"chapters/ch_{ch:02d}.md"],
                            stderr=subprocess.DEVNULL, cwd=str(BASE_DIR), shell=False
                        )
                    except Exception:
                        pass
...
def run_revision(state: dict, max_cycles: int = MAX_REVISION_CYCLES) -> dict:
    ...
    for cycle in range(start_cycle, max_cycles + 1):
        ...
        apply_cuts = BASE_DIR / "scripts/apply_cuts.py"
        if apply_cuts.exists():
            step("Applying mechanical cuts (OVER-EXPLAIN, REDUNDANT)...")
            run_tool("uv run python scripts/apply_cuts.py all "
        ...
        for idx, item in enumerate(consensus_items):
            ...
            gen_brief = BASE_DIR / "scripts/gen_brief.py"
            if gen_brief.exists():
                step(f"Generating brief for Ch {ch_num}...")
                run_tool(f"uv run python scripts/gen_brief.py --panel {ch_num}", timeout=300)
    ...
    # =========================================================
    review_py = BASE_DIR / "scripts/review.py"
    if review_py.exists():
        ...
        for rnd in range(1, max_review_rounds + 1):
            ...
            parse_result = run_tool(
                "uv run python scripts/review.py --parse", timeout=60)
            ...
            gen_brief_py = BASE_DIR / "scripts/gen_brief.py"
            if gen_brief_py.exists():
                # Auto mode: picks weakest chapter, cross-references all sources
                run_tool("uv run python scripts/gen_brief.py --auto", timeout=300)
            ...
            apply_cuts_py = BASE_DIR / "scripts/apply_cuts.py"
            if apply_cuts_py.exists():
                run_tool(
                    "uv run python scripts/apply_cuts.py all --types OVER-EXPLAIN REDUNDANT --min-fat 15",
                    timeout=300)
