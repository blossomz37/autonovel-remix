...
def run(cmd, timeout=600, cwd=None):
    ...
    r = subprocess.run(cmd_args, shell=False, capture_output=True, text=True, timeout=timeout, cwd=cwd)
    return r.stdout + r.stderr, r.returncode

def slop_check(ch):
    out, _ = run(f'.venv/bin/python3 -c "from scripts.evaluate import slop_score, load_file; import json; r=slop_score(load_file(\'chapters/ch_{ch:02d}.md\')); print(json.dumps(r))"')
...
def pattern_check(ch):
    ...
    with open(f"chapters/ch_{ch:02d}.md", "r", encoding="utf-8") as f:
        words = len(f.read().split())
    return words, didnot, thought

def spot_eval(ch):
    out, rc = run(f'.venv/bin/python3 scripts/evaluate.py --chapter={ch}', timeout=300)
...
for ch in chapters:
    ...
    # Draft
    out, rc = run(f'.venv/bin/python3 scripts/draft_chapter.py {ch}')
    if rc != 0:
    ...
    # Git commit
    run(["git", "add", f"chapters/ch_{ch:02d}.md", "state.json"], cwd="/home/jeffq/autonovel")
